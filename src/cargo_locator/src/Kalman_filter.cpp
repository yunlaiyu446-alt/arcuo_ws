...
void StaticPoseEKF::update(const MeasurementVector& measurement){
    ...
    MeasurementVector aligned_measurement = measurement;
    const double quaternion_norm = aligned_measurement.segment<4>(3).norm();
    if (quaternion_norm > 1e-12) {
        aligned_measurement.segment<4>(3) /= quaternion_norm;
    } else {
        RCLCPP_WARN(rclcpp::get_logger("ekf"), "测量四元数范数接近零，跳过本次 EKF 更新。");
        return;
    }

    // q 和 -q 表示同一旋转。更新前将测量四元数对齐到状态所在半球。
    if (aligned_measurement.segment<4>(3).dot(state_.segment<4>(3)) < 0.0) {
        aligned_measurement.segment<4>(3) *= -1.0;
    }
    ...
    // 计算y
    MeasurementVector y = aligned_measurement - H * state_;
